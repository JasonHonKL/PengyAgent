    ...
    impl Agent {
            ...
            F: Fn(AgentEvent) + Send + Sync + 'static,
        {
            ...
            while step < self.max_step {
                step += 1;
                callback(AgentEvent::Step {
                    step,
                    max_steps: self.max_step,
                });

                // Check if previous tool call was vision_judge
                let previous_was_vision_judge = self
                    .messages
                    .iter()
                    .rev()
                    .find(|msg| {
                        matches!(msg.role, Role::Assistant)
                            && msg.content.starts_with("Tool call: vision_judge")
                    })
                    .is_some();
                ...
                if previous_was_vision_judge {
                    ...
                    for msg in self.messages.iter().rev() {
                        if matches!(msg.role, Role::User)
                            && msg.content.starts_with("Tool result: ")
                        {
                            let result = msg
                                .content
                    ...
                    if let Some(image_url) = image_data_url {
                        callback(AgentEvent::VisionAnalysis {
                            status: "Analyzing image...".to_string(),
                        });

                        // Check if the model base URL is OpenRouter
                        if !self.model.base_url.contains("openrouter.ai") {
                            callback(AgentEvent::Error {
                                error: "Vision not supported for non-OpenRouter models".to_string(),
                            });
                            ...
                            let vision_messages = vec![
                                ...
                                Message::new(
                                    Role::User,
                                    "here's the summary of this image".to_string(),
                                ),
                            ];
                            ...
                            match self
                                .model
                                .open_router_vision_completion(image_url, vision_messages)
                                .await
                            {
                                Ok(summary) => {
                                    callback(AgentEvent::VisionAnalysis {
                                        status: "Image analyzed".to_string(),
                                    });
                                    // Add the summary as a user message
                                    self.messages.push(Message::new(
                                        Role::User,
                                        format!("Image summary: {}", summary),
                                    ));
                                }
                                Err(e) => {
                                    callback(AgentEvent::Error {
                                        error: format!("Failed to summarize image: {}", e),
                                    });
                                    // Continue anyway, but add an error message
                                    self.messages.push(Message::new(
                                        Role::User,
                                        format!("Failed to summarize image: {}", e),
                                    ));
                                }
                            }
                        }
                        ...
                        .await
                    {
                        Ok((messages, usage)) => {
                            ...
                            for msg in messages.iter().rev() {
                                if matches!(msg.role, Role::Assistant)
                                    && msg.content.starts_with("Tool call:")
                                {
                                    ...
                                    if let Some(tool_info) = msg.content.strip_prefix("Tool call: ")
                                    {
                                        ...
                                        let (tool_name, args, parse_success) =
                                            match serde_json::from_str::<serde_json::Value>(
                                                ...
                                                Err(_e) => {
                                                    ...
                                                    if let Some((name, args)) =
                                                        tool_info.split_once(" with arguments: ")
                                                    {
                                                        ...
                                                        {
                                                            let after_name =
                                                                &tool_info[name_idx + 6..];
                                                            if let Some(colon_idx) =
                                                                after_name.find(':')
                                                            {
                                                                let value_part = &after_name
                                                                    [colon_idx + 1..]
                                                                    .trim_start();
                                                                if value_part.starts_with('"') {
                                                                    // Find the closing quote, handling escaped quotes
                                                                    let chars = value_part[1..]
                                                                        .char_indices();
                                                                    let mut end_idx = 0;
                                                        ...
                                                        if let Some(args_idx) =
                                                            tool_info.find("\"arguments\"")
                                                        {
                                                            let after_args =
                                                                &tool_info[args_idx + 11..];
                                                            if let Some(colon_idx) =
                                                                after_args.find(':')
                                                            {
                                                                let value_part = &after_args
                                                                    [colon_idx + 1..]
                                                                    .trim_start();
                                                                if value_part.starts_with('"') {
                                                        ...
                                                        if !extracted_name.is_empty() {
                                                            ...
                                                            let found_tool = known_tools
                                                                .iter()
                                                                .find(|tool| {
                                                                    tool_info.contains(*tool)
                                                                })
                                                                .map(|s| s.to_string());
                                                            ...
                                                            if let Some(tool) = found_tool {
                                                                ...
                                                                eprintln!(
                                                                    "Critical: Could not parse tool call: {}",
                                                                    tool_info
                                                                );
                                                                (
                                                                    "unknown".to_string(),
                                        ...
                                        if !parse_success && tool_name == "unknown" {
                                            ...
                                            eprintln!(
                                                "Tool call parsing failed critically - tool: {}, args: {}. Retrying...",
                                                tool_name, args
                                            );
                                        ...
                                        if !parse_success && tool_name != "unknown" {
                                            eprintln!(
                                                "Tool call parsing warning - tool: {}, args: {}",
                                                tool_name, args
                                            );
                                        }

                                        callback(AgentEvent::ToolCall { tool_name, args });
                                    }
                            ...
                            if parsing_failed_critically {
                                ...
                                if retry_count >= self.max_retry {
                                    callback(AgentEvent::Error {
                                        error: format!(
                                            "Tool call parsing failed after {} retries",
                                            self.max_retry
                                        ),
                                    });
                                ...
                                callback(AgentEvent::Error {
                                    error: format!(
                                        "Tool call parsing failed (retry {}/{})",
                            ...
                            if found_tool_call {
                                ...
                                for msg in messages.iter().rev() {
                                        ...
                                        && msg.content.starts_with("Tool call: ")
                                    {
                                        if let Some(tool_info) =
                                            msg.content.strip_prefix("Tool call: ")
                                        {
                                            // Try to parse as JSON first (new format) with robust error handling
                                            tool_name = match serde_json::from_str::<
                                                serde_json::Value,
                                            >(
                                                tool_info.trim()
                                            ) {
                                ...
                                if tool_name.as_deref() == Some("end")
                                    || tool_name.as_deref() == Some("endtool")
                                    || tool_result
                                        .as_deref()
                                        .map(|r| r.starts_with("END_CONVERSATION"))
                                        .unwrap_or(false)
                                {
                                    let content = if let Some(result) = tool_result {
                                        if let Some(reason) =
                                            result.strip_prefix("END_CONVERSATION: ")
                                        {
                                ...
                                if tool_name.as_deref() == Some("summarizer")
                                    && tool_result.as_deref() == Some("SUMMARIZE_CONVERSATION")
                                {
                                    ...
                                    match self.summarize_conversation().await {
                                        Ok(summarized_messages) => {
                                            ...
                                            callback(AgentEvent::ToolResult {
                                                result: "Conversation summarized successfully"
                                                    .to_string(),
                                            });
                                        }
                                        Err(e) => {
                            ...
                            if let Some(last_msg) = messages.last() {
                                ...
                                if matches!(last_msg.role, Role::Assistant)
                                    && content_trimmed.starts_with("Tool call:")
                                {
                                    // This is a text-based tool call that needs to be executed manually
                                    if let Some(tool_info) =
                                        content_trimmed.strip_prefix("Tool call: ")
                                    {
                                        ...
                                        match serde_json::from_str::<serde_json::Value>(
                                            ...
                                            Ok(json) => {
                                                ...
                                                if let Some(tool) = tools_slice.and_then(|tools| {
                                                    ...
                                                    match tool.run(arguments) {
                                                        Ok(result) => {
                                                            ...
                                                            let mut updated_messages =
                                                                messages.clone();
                                                            updated_messages.push(Message::new(
                                                                Role::User,
                                                    ...
                                                    updated_messages.push(Message::new(
                                                        Role::User,
                                                        format!("Tool result: {}", error_result),
                                                    ));
                                                    self.messages = updated_messages;
                                                    break Ok(());
                                                }
                                            ...
                                            Err(e) => {
                                                ...
                                                let error_result = format!(
                                                    "Tool error: Failed to parse tool call JSON: {}",
                                                    e
                                                );
                                                callback(AgentEvent::ToolResult {
                                                    result: error_result.clone(),
                            ...
                            if let Some(last_msg) = messages.last() {
                                    ...
                                    && !content_trimmed.starts_with("Tool call:")
                                {
                                    // Final response
                                    self.messages = messages.clone();
                                    callback(AgentEvent::FinalResponse {
                        ...
                        Err(e) => {
                            ...
                            if retry_count >= self.max_retry {
                                callback(AgentEvent::Error {
                                    error: format!("Error after {} retries: {}", self.max_retry, e),
                                });
                                break Err(e);
                            }
                            callback(AgentEvent::Error {
                ...
                if result.is_err() {
                    callback(AgentEvent::Error {
                        error: "Failed to complete after retries".to_string(),
                    });
                    return;
                }
                ...
                if let Some(last_msg) = self.messages.last() {
                    ...
                    if matches!(last_msg.role, Role::User)
                        && last_msg.content.starts_with("Tool result: ")
                    {
                        continue;
                    }
            ...
            if step >= self.max_step {
                callback(AgentEvent::Error {
                    error: format!("Reached maximum steps ({})", self.max_step),
                });
            }
        }
