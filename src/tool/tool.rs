pub mod tool {
    ...
    impl ToolCall for Tool {
        fn get_json(&self) -> Result<serde_json::Value, serde_json::Error> {
            ...
            for (key, param) in &self.parameters {
                ...
                prop.insert(
                    "type".to_string(),
                    serde_json::Value::String(param_type.clone()),
                );
                prop.insert(
                ...
                if let Some(ref enum_vals) = param.enum_values {
                    let enum_array: Vec<serde_json::Value> = enum_vals
                        .iter()
                    ...
                    for (k, v) in &param.items {
                        if k != "type" && k != "item_type" {
                            // Skip type and item_type as they're already handled
            ...
            if !self.required.is_empty() {
                let required_array: Vec<serde_json::Value> = self
                    .required
                    .iter()
            ...
            function_obj.insert(
                ...
                serde_json::Value::Object(parameters_obj),
            );
    ...
    mod tests {
        ...
        fn test_tool_to_json() {
            ...
            assert_eq!(
                function.get("name").unwrap().as_str().unwrap(),
                "get_weather"
            );
            assert_eq!(
                function.get("description").unwrap().as_str().unwrap(),
                "Get the weather"
            );
