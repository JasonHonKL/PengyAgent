pub mod grep {
    ...
    impl GrepTool {
        pub fn new() -> Self {
            ...
            parameters.insert("pattern".to_string(), Parameter {
                items: pattern_items,
                description: "The regular expression pattern to search for in file contents.".to_string(),
                enum_values: None,
            });
        ...
        fn execute_grep(&self, pattern: &str, path: Option<&str>, include: Option<&str>) -> Result<String, Box<dyn Error>> {
            ...
            match output {
                Ok(result) => {
                    ...
                    if result.status.code() == Some(1) && stdout.is_empty() {
                        return Ok(format!("No matches found for pattern '{}' in {}", pattern, search_path));
                    }
        ...
        fn fallback_to_grep(&self, pattern: &str, path: &str, include: Option<&str>) -> Result<String, Box<dyn Error>> {
            ...
            if output.status.code() == Some(1) && stdout.is_empty() {
                return Ok(format!("No matches found for pattern '{}' in {}", pattern, path));
            }
            
            if !output.status.success() && output.status.code() != Some(1) {
                return Err(format!("grep command failed with exit code {}: {}", 
                    output.status.code().unwrap_or(-1), stderr).into());
            }
            ...
            if result_str.trim().is_empty() {
                Ok(format!("No matches found for pattern '{}' in {}", pattern, path))
            } else {
                Ok(result_str.trim().to_string())
            }
        }
